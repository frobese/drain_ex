defmodule DrainEx.Protocol do
  ...
  def decode(data) do
    case CBOR.decode(data) do
      ...
      {:ok, msg, rest} ->
        Logger.warn(
          "CBOR decode short length, ignoring #{byte_size(rest)} of #{byte_size(data)} bytes"
        )
  ...
  defp reduce_keys_to_atoms({key, %CBOR.Tag{tag: :bytes, value: bin}}),
    do: {String.to_atom(key), bin}
